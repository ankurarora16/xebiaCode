...
var SortableTblPager = function (_React$Component) {
    ...
    var elemArr= Array.apply(null, {length: this.props.totalPage}).map(Number.call, Number);
  //   console.log("Array is"+ elemArr);
	// console.log("Total pages are: "+ this.props.totalPage+" "+ this.state.currPage);
		return _react2.default.createElement(
			'div',
			{ className: 'form-group' },
			_react2.default.createElement(
				'div',
				{ className: 'form-inline pull-right formMargin' }, 
				_react2.default.createElement('input', { type: 'button', className: 'btn-default', name: '', disabled: prevDisableStyle,
					onClick: this.subPage, value: 'Prev' }),
				_react2.default.createElement(
					'select',
          { onChange: this.setCurrentPage, value: this.state.currPage,
             className: 'form-control page-select' },
             elemArr.map(function (x, i) {
						return _react2.default.createElement(
							'option',
							{ key: i, value: x },
							x+1
				...
				_react2.default.createElement('input', { type: 'button', className: 'btn-default', name: '', disabled: nextDisableStyle,
...
var SortableTbl = function (_React$Component) {
    ...
    console.log("newAsc"+ JSON.stringify(newAsc));
    console.log("newData"+ JSON.stringify(newData));
    console.log("dKey"+ JSON.stringify(dKey));

		newData.sort(function (a, b) {
      if(dKey === "closingDate"){
        if(a[dKey])
          {
          var from1 = a[dKey].split("/");
          var date1 = new Date(from1[2], from1[1] - 1, from1[0]);}
          else
          var date1= new Date();
          if(b[dKey]){
          var from2 = b[dKey].split("/");
           var date2 = new Date(from2[2], from2[1] - 1, from2[0]);
          }
          else
          var date2= new Date();
        if (date1 === date2) return 0;
			if (nAsc ? date1 > date2 : date1 < date2) return 1;
      if (nAsc ? date1 < date2 : date1> date2) return -1;
      return 0
      }
else{
